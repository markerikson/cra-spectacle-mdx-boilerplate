...
const MODREG = /^(import|export)(?! default class MDXContent)\s/
const SLIDEREG = /\n---\n/
...
const defaultLayout = (str) => {
  if (!hasDefaultExport(str)) {
    str = `import {DefaultSlide} from 'components/slides'
...
module.exports = async function (src) {
  ...
  const slides = normalizeNewline(content)
    ...
    .map(str => {
        const mdxString = mdx.sync(str, options)
        return mdxString
    })
    .map(str => str.trim())
    ...
    .map(str => {
      const lines = str.split('\n')
      const matchingInlineModules = [...lines.filter(str => MODREG.test(str))]

      inlineModules.push(
          ...matchingInlineModules
      );
      const nonMatchingInlineModules = lines.filter(str => !MODREG.test(str))
        .filter(Boolean)
        .join('\n')

        return nonMatchingInlineModules;
    })
